...
- 1-GPU **GRES** (`gres.conf`: Slurm’s map of which GPU device files and CPU cores exist) — `/dev/nvidia0`, `Cores=0-15`. Stock map is 8 GPUs / `Cores=0-31` and crashes `slurmctld` on 16-CPU nodes.
