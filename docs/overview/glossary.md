...
| **Nodesets** | Kubernetes **worker** node groups mapped to Slurm roles: system, controller, login, accounting, NFS, GPU workers. None of these is the Kubernetes master. MK8s keeps the control plane (API server, etcd) as a managed service you do not schedule onto. |
...
| **Controller** | Kubernetes worker that runs `slurmctld` (the Slurm scheduler). “Controller” is a Slurm term, not a Kubernetes master. |
| **Accounting** | Kubernetes worker that runs **slurmdbd** (Slurm accounting) and MariaDB. Assignment: 1 × `8vcpu-32gb`. Job history in `sreport` / `sacct` lives here. |
| **NFS** | Kubernetes worker that runs an in-cluster NFS server (`nfs_in_k8s`). Assignment: 1 × `4vcpu-16gb`. Separate from the jail filestore and `/mnt/data`. |
...
| --- | --- |
| GPU `Deployment` | Training: `sbatch` in `soperator`. Serving: scale workers to 1 (`task-2/02-serve.sh`) then vLLM in `task2-inference` |
...
Soperator worker pods occupy the GPUs (device plugin + taint). A second `Deployment` requesting `nvidia.com/gpu` stays `Pending` until workers are scaled to 1 (`task-2/02-serve.sh`).
