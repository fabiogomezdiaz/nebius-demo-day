...
if [[ -z "${KEY}" ]]; then
  ...
  echo "If login-host is omitted, it is read from terraform/workloads via terraform state." >&2
  exit 1
fi

if [[ -z "${LOGIN_HOST}" ]]; then
  INSTALL_DIR="${ROOT}/terraform/workloads"
  if [[ ! -d "${INSTALL_DIR}" ]]; then
    echo "No workloads dir at ${INSTALL_DIR}. Pass the login host explicitly." >&2
    exit 1
  fi
  LOGIN_HOST="$(
